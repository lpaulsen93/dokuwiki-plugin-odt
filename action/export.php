...
class action_plugin_odt_export extends DokuWiki_Action_Plugin {
    ...
    protected function collectExportPages(Doku_Event $event) {
        ...
        if($ACT == 'export_odt') {
            ...
            if(!$title = $INPUT->str('export_title')) {
                $this->showPageWithErrorMsg($event, 'needtitle');
                return false;
            }
            $docnamespace = cleanID($INPUT->str('export_ns'));
            ...
            //sort order
            $order = $INPUT->str('export_order', 'natural', true);
            ...
            //search depth
            $depth = $INPUT->int('export_depth', 0);
            if($depth < 0) {
                $depth = 0;
            }
